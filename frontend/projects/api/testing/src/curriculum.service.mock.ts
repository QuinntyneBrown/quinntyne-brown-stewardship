...
export class CurriculumServiceMock implements ICurriculumService {
  ...
  async getModule(ordinal: number | null) {
    ...
    if (!curriculum.isProgrammePublished) throw new ServiceError(409, undefined, 'This programme is not yet available.');
    const module = this.store.module(ordinal ?? curriculum.currentOrdinal ?? this.store.lastOrdinal());
    if (!module) throw new ServiceError(404, undefined, 'Module not found.');
    if (!module.isComplete && module.ordinal !== curriculum.currentOrdinal) throw new ServiceError(409, undefined, 'Complete the current module to unlock this module.');
    return module;
  }
