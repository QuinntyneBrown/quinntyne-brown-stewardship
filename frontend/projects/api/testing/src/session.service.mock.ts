...
export class SessionServiceMock implements ISessionService {
  ...
  async preparation(id: string) { const session = await this.get(id); const module = this.store.module(session.moduleOrdinal ?? 12); const page = this.store.notePage(undefined, id); return { moduleId: module.id, session, moduleOrdinal: module.ordinal, moduleTitle: module.title, prompts: module.preparationPrompts, notes: page.notes, notesCursor: page.nextCursor }; }
