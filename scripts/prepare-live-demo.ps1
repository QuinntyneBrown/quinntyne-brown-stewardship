...
Invoke-DemoCli @('import-curriculum', 'backend/src/QuinntyneBrownStewardship.Cli/Content/starter-curriculum.json')
...
foreach ($definition in @(
    @{id=$cohort; startDate=(Get-Date).AddDays(-28).ToString('yyyy-MM-dd'); mentorEmail='mentor@demo.invalid'},
    @{id=$endedCohort; startDate=(Get-Date).AddDays(-100).ToString('yyyy-MM-dd'); mentorEmail='mentor@demo.invalid'}
)) {
...
DECLARE @module uniqueidentifier = (SELECT Id FROM Modules WHERE Ordinal = 1);
