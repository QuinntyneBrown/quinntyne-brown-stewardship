export class SessionsPage extends ShellPage {
  async ready() {
    await this.page
      .getByRole("heading", { name: "Sit with your mentor" })
      .waitFor();
  }
  async canBook() {
    await this.page
  ...
  async history() {
    ...
    await this.canBook();
    const notes = this.page.getByRole("link", {
      name: "Session notes",
      exact: true,
    });
    if ((await notes.count()) !== 2)
      throw new Error("The two historical sessions are missing.");
    await notes.first().click();
    await this.text("Show all notes");
    await this.text(
      "Conversation reflection: listen to the people carrying the cost before choosing a technical solution.",
    );
    await this.pause(2600);
    await this.go("/sessions");
    await this.canBook();
    await this.publishedDay();
  }
  async publishedDay() {
    const name = new Intl.DateTimeFormat("en-CA", {
      ...
      day: "numeric",
      timeZone: "UTC",
    }).format(new Date(this.config.firstDay + "T12:00:00Z"));
    let target = this.page
      .getByRole("group", { name: "Booking days" })
    ...
    if (
      ...
      throw new Error("A taken slot is selectable.");
    await this.page
  ...
  async book() {
    ...
    if (!(await button.isDisabled()))
      throw new Error("Booking is offered before selecting a slot.");
    await this.page
      .getByRole("button", { name: /Open ·/ })
      .first()
      .click();
    ...
    const response = this.page.waitForResponse(
      (r) =>
        r.url().split("?")[0] === this.config.baseUrl + "/sessions" &&
        r.request().method() === "POST" &&
        r.status() === 200,
    );
    await button.click();
    this.page.__demo.booking = await (await response).json();
    await this.page
  ...
  async changeAndCancel() {
    ...
    if (changed.slotId === this.page.__demo.booking.slotId)
      throw new Error("Rescheduling did not change the booking.");
  ...
  async cutoff() {
    await this.go("/sessions");
    const change = this.page.getByRole("button", {
      name: "Reschedule",
      exact: true,
    });
    ...
    if (
      ...
      throw new Error("The 24-hour change cutoff is not enforced in the UI.");
    await this.text("Changes close 24 hours before the session starts.");
  ...
  async ended() {
    ...
    if (
      ...
      throw new Error("An ended cohort can book another session.");
  }
}
