export class SignInPage extends ShellPage {
  ...
  async enterKeyboard() {
    ...
    await this.page.keyboard.type(this.config.email, { delay: 25 });
    await this.page.keyboard.press("Tab");
    await this.page.keyboard.type(this.config.password, { delay: 15 });
    await this.page.keyboard.press("Tab");
  ...
  async validation() {
    ...
    await this.page
      .getByRole("button", { name: "Sign in", exact: true })
      .click();
    await this.text("Enter your email address.");
    await this.text("Enter your password.");
  ...
  async awaiting() {
    ...
    await this.page
      .getByRole("heading", { name: "You are not yet enrolled in a cohort." })
      .waitFor();
  ...
  async protectedAfterSignOut() {
    ...
    const response = await this.page.request.get(
      this.config.baseUrl + "/notes",
    );
    if (response.status() !== 401)
      throw new Error("Signed-out API access was not refused.");
    this.page.__demo.checks.push(
      "Signed-out notes API returns 401; browser history and direct URLs return to sign-in.",
    );
  }
}
