...
public sealed class StewardshipDbContext(DbContextOptions<StewardshipDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder model)
    {
        ...
        model.Entity<Cohort>(b => { b.Property(x => x.CurriculumKey).HasMaxLength(100); b.Property(x => x.TimeZone).HasMaxLength(100); b.HasOne<Participant>().WithMany().HasForeignKey(x => x.MentorId).OnDelete(DeleteBehavior.Restrict); });
        model.Entity<CurriculumModule>(b => { b.HasKey(x => x.Id); b.Property(x => x.CurriculumKey).HasMaxLength(100); b.Property(x => x.Title).HasMaxLength(254); b.HasIndex(x => new { x.CurriculumKey, x.Ordinal }).IsUnique(); b.HasMany(x => x.Sections).WithOne().HasForeignKey(x => x.ModuleId).OnDelete(DeleteBehavior.Restrict); b.HasMany(x => x.PreparationPrompts).WithOne().HasForeignKey(x => x.ModuleId).OnDelete(DeleteBehavior.Restrict); });
        model.Entity<ModuleSection>(b => { b.HasKey(x => x.Id); b.HasIndex(x => new { x.ModuleId, x.Ordinal }).IsUnique(); });
